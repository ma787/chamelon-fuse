...
let get_elems x = x.fd, x.seek_offset, x.m, x.info, x.config, x.use_fsync_after_write
...
let of_fd path fd =
  let config = Config.create path in
  let use_fsync_after_write = (not config.buffered) && is_win32 in
  get_file_size path fd >>= function
    | Error (`Msg e) ->
      Unix.close fd;
      fail_with e
    | Error _ -> fail_with "mirage-block-unix:of_config: unknown error"
    | Ok size_bytes ->
      get_sector_size ?prefered_sector_size:config.prefered_sector_size path fd >>= function
      | Error (`Msg e) ->
        Unix.close fd;
        fail_with e
      | Error _ -> fail_with "mirage-block-unix:of_config: unknown error"
      ...
      | Ok sector_size ->
        ...
        let seek_offset = 0L in
        return ({ fd = Some fd; seek_offset; m;
                  info = { Mirage_block.sector_size; size_sectors; read_write = true };
                  config; use_fsync_after_write })
